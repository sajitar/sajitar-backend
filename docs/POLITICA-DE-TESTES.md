...
| **Integração (API HTTP)** | Validar contratos de endpoints (status, corpo JSON, validação, i18n `lang`) com a aplicação em contexto Spring e PostgreSQL. | `ProfileControllerIntegrationTest` (`/profiles`) e `CheckerControllerIntegrationTest` (`/checkers`) (`@SpringBootTest` + MockMvc). Novo endpoint = sucesso + 404/400/409 (e 403 de checker) no IT do recurso. CI com **PostgreSQL** (`postgres:latest` como serviço) antes de `./mvnw verify` — ver [`.github/workflows/branch-policy.yml`](../.github/workflows/branch-policy.yml). |
